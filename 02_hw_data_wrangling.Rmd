...
There are 7014 candidates as per the information in the table
...
party_incumbent_promote <- party_incumbent_promote  |>
...
knitr::kable(party_incumbent_promote, col.names = c("Party", "Incumbency", "Proportion"), digits = 3)


```

Using the data given, the type of candidate that sponsored the most promoting ads on average were Incumbent Democrats.
...
plot_q3 <- ggplot(data = fb_ads |> 
                    filter(office == "US House"), mapping = aes(x = impressions_millions)) +
  geom_histogram()
...
As per the data, a majority of the candidates for the US House did not have over 10 million ad impressions on Facebook
...
fb_top_plot <- ggplot(data = fb_ads|> 
  ...
  mutate(cand_name_last = reorder(cand_name_last,impressions_millions)), 
                      mapping = aes(x=impressions_millions,y=cand_name_last)) + 
  geom_bar(stat = "identity", aes(fill = party)) + labs(x = "Impressions (in Millions)", y = "Candidate Surname", 
